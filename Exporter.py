...
class Exporter:
    ...
    def _register_cycles_renderer_material(self, material, mat_id):
        ...
        mat_desc['opacity'] = material.alpha_threshold
    ...
    def run(self, context,
        ...
        if export_model:
            for obj_idx, obj in enumerate(objects_to_export):
                try:
                    mesh_settings = 'RENDER' if use_mesh_modifiers_render else 'PREVIEW'
                ...
                per_vertex_attribute_desc = [OrderedDict([
                ...
                if export_normals:
                    per_vertex_attribute_desc.append(OrderedDict([
                ...
                if export_uv_maps:
                    ...
                    num_uv_layers = min(8, len(mesh.uv_layers))
                ...
                for poly in mesh.polygons:
                    ...
                    material_id = self.get_material_id(materials[poly.material_index])
                ...
                self.meshes.append(OrderedDict([
                    ('attributes', per_vertex_attribute_desc),
                    ('vertices', vertex_attributes),
                    ('parts', local_parts)
                ]))
                self.nodes.append(OrderedDict([
                    ('id', obj.name),
                    ('skeleton', False),
                    ('transform', Table([col for row in transform.transposed() for col in row], 4)),
